...
tps <- tps |> 
  ...
  mutate(first_app_type = first(form_type),
...
# For most recent time period, we just check if an individual has had approved TPS within 6-18 months of date
...
# TO DO: For historical estimates, get all records with first approval before given date and latest approval after

report_dates <- unique(crs_long$report_date)

report_dates_minus_6 <- report_dates - days(182)
report_dates_minus_12 <- report_dates - days(364)
report_dates_minus_18 <- report_dates - days(546)
